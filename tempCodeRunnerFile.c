...
unsigned char nt_to_bits(char nt) {
    switch(nt) {
        ...
        default: return 0x03; /* T or anything else -> 11 */
    }
}
...
unsigned char *validate_and_strip_checksums(const unsigned char *data, size_t nbytes_with_checks, size_t *out_nbytes) {
    ...
    while(inpos < nbytes_with_checks) {
        ...
        for(i=0;i<BLOCK_BYTES && (inpos + i) < nbytes_with_checks - 0; i++) {
            ...
            if(inpos + i + 1 >= nbytes_with_checks) break;
        ...
        inpos += i;
        if(inpos >= nbytes_with_checks) { error = 1; break; } /* missing checksum */
        unsigned char stored = data[inpos++];
        if(stored != checksum) { error = 1; break; }
    }
    if(error) { free(outbuf); return NULL; }
    *out_nbytes = outpos;
    return outbuf;
}
...
int main(void) {
    ...
    unsigned char key[] = {0x5A, 0xA5}; size_t keylen = sizeof(key);
    ...
    if(!decoded_with_checks) { printf("Decoding failed (dna->bytes)\n"); goto cleanup; }
